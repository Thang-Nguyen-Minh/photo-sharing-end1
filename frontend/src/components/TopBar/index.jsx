...
function TopBar() {
    ...
    useEffect(() => {
        const fetchUserInfo = async () => {
            try {
                const response = await axios.get("http://localhost:8080/admin/loginUser", { withCredentials: true });
    ...
    const handleLogout = (event) => {
        ...
        if (isLoggedIn) {
            axios.get("http://localhost:8080/admin/logout", { withCredentials: true })
                .then((response) => {
